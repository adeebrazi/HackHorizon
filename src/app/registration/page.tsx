...
export default function RegistrationForm() {
  ...
  const [team, setTeam] = useState<{
    teamName: string;
    collegeName: string;
    leader: { name: string; email: string; phone: string; department: string; year: string };
  ...
  const handleMemberCount = (count: number) => {
    ...
    setTeam((prev) => ({
      ...
      members: Array(count - 1)
        .fill(0)
        .map((_, i) => prev.members[i] || { name: "", email: "", phone: "", department: "", year: "" }),
    }));
  };
  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const fetchPromise = fetch("/api/register", {
        method: "POST",
        body: formData,
      });
        ...
        <>
          {!regType ? (
            <div className="relative z-10 container mx-auto px-4 py-8 min-h-screen flex flex-col items-center justify-center animate-fadeIn">
              ...
              <div className="w-full max-w-xl bg-gradient-to-b from-[#1a1206] to-[#0a0a0a] rounded-xl shadow-2xl border-2 sm:border-4 border-[#4C1D00] p-6 sm:p-8 space-y-6 text-center"
                style={{
                  boxShadow: "0 0 30px rgba(232, 192, 104, 0.4), inset 0 0 20px rgba(0,0,0,0.6)",
                }}
              >
                <h2 className="text-2xl sm:text-3xl font-bold text-[#E8C068] text-3d mb-2">Select Category</h2>
                <p className="text-gray-400 text-sm sm:text-base mb-6">Choose your team type to proceed with the registration.</p>
                ...
                <div className="space-y-4">
                  <button
                    ...
                    onClick={() => {
                      playClickSound();
                      setRegType("regular");
                      setTeam((prev) => ({ ...prev, registrationType: "regular" }));
                    }}
                  ...
                  >
                    <span className="text-lg sm:text-xl font-bold text-[#FFD700] group-hover:text-[#E8C068] transition-colors">1. REGULAR REGISTRATION</span>
                    <span className="text-gray-300 text-xs sm:text-sm">
                      (Teams consisting of members from multiple programs (B.Tech, Diploma, BCA))
                    </span>
                  </button>
                  ...
                  <button
                    ...
                    onClick={() => {
                      playClickSound();
                      setRegType("diploma");
                      setTeam((prev) => ({ ...prev, registrationType: "diploma" }));
                    }}
                    className="w-full p-5 sm:p-6 bg-[#0a0a0a] border-2 border-[#4C1D00] rounded-lg text-left hover:border-[#FFD700] transition-all transform hover:scale-[1.02] active:scale-[0.98] group flex flex-col gap-1 cursor-pointer"
                    style={{
                      boxShadow: "0 4px 0 rgba(0,0,0,0.5)",
                  ...
                  >
                    <span className="text-lg sm:text-xl font-bold text-[#FFD700] group-hover:text-[#E8C068] transition-colors">2. DIPLOMA REGISTRATION</span>
                    <span className="text-gray-300 text-xs sm:text-sm">
            ...
            {currentStep === 2 && (
              <div className="space-y-6 animate-fadeIn">
                ...
                <div className="space-y-4">
                  ...
                  <div>
                    <label className="block text-[#E8C068] font-bold mb-2 text-lg">Leader Department</label>
                    <input
                      type="text"
                      name="department"
                      required
                  ...
                  <div>
                    <label className="block text-[#E8C068] font-bold mb-2 text-lg">Leader Year</label>
                    <select
                      name="year"
                      required
          ...
          <div className="bg-[#0a0a0a] p-4 sm:p-6 border-t-2 sm:border-t-4 border-[#4C1D00] flex justify-between items-center">
            {!success && (currentStep > 1 || (currentStep === 1 && regType)) && (
              <button
                type="button"
                onClick={() => {
                  playClickSound();
                  if (currentStep === 1) {
                    setRegType(null);
                    setTeam((prev) => ({ ...prev, registrationType: "" }));
                  } else {
                    prevStep();
                  }
                }}
