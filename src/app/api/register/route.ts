...
export async function POST(req: NextRequest) {
  try {
    ...
    // For now, only upload to cloudinary
    const paymentScreenshotUrl = paymentFile ? await uploadToCloudinary(paymentFile) : "";
    
    console.log("File upload completed");
    
    // Google Sheets functionality commented out for now
    // Create sheets client and timestamp
    // const sheets = google.sheets({ version: "v4", auth });
    const now = new Date().toLocaleString("en-IN", {
      timeZone: "Asia/Kolkata",
      year: "numeric",
    ...
    // Google Sheets row data (commented out)
    // const row = [
    //   now,
    //   teamName,
    //   collegeName,
    ...
    // Google Sheets write operation (commented out)
